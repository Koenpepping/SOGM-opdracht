...
//door:						Koen Pepping
	...
	/* eerste argument (pointer) wijst later naar het beginadres van de array d en zal dus het eerste element uit de array terug geven*/
	int getal;
...
// pointer naar functie:
	...
	op = telOp; // wijst nu naar de functie telOp
	op(getal, getal2);
}
...
int zoek( int * t, int lengte) //  
{
	...
	{
		if (*t == getal) // check of getal overeenkomt met het element waar de pointer *t naar wijst
			return *t; 
		t++; // *t wijst naar eerste element van d (array) met t++ springt hij door naar het volgende element uit de array 
	}
	return 0;
}
